...
from key_boards import support_keyboard, MyCallback
...
async def command_start_handler(message: Message, state: FSMContext) -> None:
    if message.chat.id == GROUP_CHAT_ID:
        await message.answer(
            f"Привет, {message.chat.full_name}! "
...
async def process_callback_button1(
    ...
    if not current_state or current_state == Support.started:
        ...
        await bot.send_message(
            callback_query.from_user.id, "У вас уже есть открытое обращение в поддержку"
        )
...
async def get_support_request(message: Message) -> None:
    await message.answer(
        "Если ты хочешь создать обращение в поддержку, пожалуйста, "
...
async def get_support_request(message: Message, state: FSMContext) -> None:
    ...
    await bot.send_message(
        GROUP_CHAT_ID,
        f"Новое обращение № {new_ticket.id} "
...
async def get_support_request(message: Message) -> None:
    await message.answer(
        "Ваш комментарий по обращению передан в поддержку. В ближайшее время я вернусь с ответом."
    )
    ticket = await session.execute(
        select(Ticket).where(
            Ticket.tg_user_id == message.from_user.id, Ticket.activ == 1
        )
    )
    ticket = ticket.scalars().first()
    await bot.send_message(
        GROUP_CHAT_ID,
        f"Комментарий обращения № {ticket.id} "
        f"от пользователя {message.from_user.full_name}: "
        f"\n{message.text}",
    )
...
async def collect_answers(message: Message):
    ...
    if message.text.lower() == "закрыть":
        ...
        state = FSMContext(
            storage=storage,
            key=StorageKey(
                chat_id=ticket.tg_user_id, user_id=ticket.tg_user_id, bot_id=bot.id
            ),
        )
        await state.set_state(Support.started)
    else:
